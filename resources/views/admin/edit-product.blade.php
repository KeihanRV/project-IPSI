...
<div class="max-w-6xl mx-auto py-8">
    ...
    <h1 class="text-3xl md:text-4xl font-bold text-center text-gray-900 mb-8 mt-4">Edit Produk</h1>
        ...
        <div class="flex flex-col md:flex-row rounded-2xl overflow-hidden shadow-sm border border-gray-200">
            ...
            <div class="w-full md:w-1/2 bg-gray-100 p-8 flex flex-col items-center justify-center max-h-[500px] relative">
                ...
                <label for="imageInput" class="cursor-pointer w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl hover:bg-gray-200 transition-colors relative overflow-hidden group aspect-square max-w-sm">
                    ...
                    <div id="imagePlaceholder" class="flex flex-col items-center justify-center {{ $product->image ? 'hidden' : '' }}">
                    ...
                    <img id="imagePreview" src="{{ $product->image ? asset('storage/product/' . $product->image) : '' }}" alt="Preview" class="absolute inset-0 w-full h-full object-cover {{ $product->image ? '' : 'hidden' }}">
                </label>
            ...
            <div class="w-full md:w-1/2 bg-[#fdf8dd] p-8 md:p-10 flex flex-col">
                ...
                <div class="space-y-6 flex-grow">
                    ...
                    <div>
                        <input type="text" name="title" placeholder="Nama Produk" required class="w-full bg-transparent border-b border-gray-400 pb-2 text-gray-800 placeholder-gray-500 focus:outline-none focus:border-yellow-600 transition-colors" value="{{ old('title', $product->title) }}">
                        @error('title') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <textarea name="description" placeholder="Deskripsi" rows="2" class="w-full bg-transparent border-b border-gray-400 pb-2 text-gray-800 placeholder-gray-500 focus:outline-none focus:border-yellow-600 transition-colors resize-none">{{ old('description', $product->description) }}</textarea>
                    </div>

                    <div>
                        <textarea name="specification" placeholder="• Spesifikasi\n• " rows="3" class="w-full bg-transparent border-b border-gray-400 pb-2 text-gray-800 placeholder-gray-500 focus:outline-none focus:border-yellow-600 transition-colors resize-none">{{ old('specification', $product->specification) }}</textarea>
                    </div>

                    <div class="flex gap-4">
                        <div class="w-1/2">
                            <input type="text" name="location" placeholder="Lokasi (ex: Jakarta Selatan)" required class="w-full bg-transparent border-b border-gray-400 pb-2 text-gray-800 placeholder-gray-500 focus:outline-none focus:border-yellow-600 transition-colors" value="{{ old('location', $product->location) }}">
                ...
                <div class="flex justify-end gap-3 mt-8">
                    <a href="{{ route('admin.dashboard') }}" class="px-6 py-2 border border-gray-300 text-gray-600 font-medium rounded-md hover:bg-gray-50 transition-colors">
                        Batal
                    </a>
    ...
    <x-confirm-modal
        ...
        message="Apakah Anda yakin ingin memperbarui data produk ini? Perubahan akan disimpan ke dalam sistem."
    />
</div>

<script>
    const DRAFT_KEY = 'admin:edit-product:{{ $product->id }}:draft';
    ...
    function initDraft() {
        ...
        populateFields({
            ...
            description: serverDraft.description ?? savedDraft?.description ?? productData.description,
            specification: serverDraft.specification ?? savedDraft?.specification ?? productData.specification,
            location: serverDraft.location ?? savedDraft?.location ?? productData.location,
        });
